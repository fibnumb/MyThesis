...
At the temperatures and pressures typical to the Universe today nuclear matter is confined to a hadronic gas.  However, it was theorized that at extreme temperatures, such as those experienced in th early Universe, partons would have undergone a phase transition where by they were no longer bound to a color neutral state.  This state of matter would have been analogous to a conventional plasma where by the electrons are no longer bound to a nucleus, thus the new state of matter was dubbed the Quark-Gluon Plasma (QGP)
...
\begin{figure}[h]
\includegraphics[width=10.0cm]{610_1}
\centering
...
By colliding heavy nuclei together in high energy collisions it is possible to obtain the energy densities and temperatures associated with the QGP state.  The first signatures for the 

\subsection{Jets as a Probe of the QGP}

\begin{figure}[h]
\includegraphics[width=5.0cm]{dijetfig2}
\centering
...
\begin{figure}[h]
\includegraphics[width=12.0cm]{ppevolution}
\centering
...
\begin{equation}
R_{AA} = \frac{1}{N_{binary}} \frac{d^{2}N_{NN}/dp_{T}d\eta}{d^{2}N_{pp}/dp_{T}d\eta}
\label{eq:RAA}
\end{equation}

\noindent
where $N_{binary}$ is the number of binary collisions and is estimated using a Glauber model\cite{Miller:2007ri} of a nucleus while $d^{2}N_{NN}/dp_{T}d\eta$ and $d^{2}N_{NN}/dp_{T}d\eta$ are the spectra measured in nucleus nuceus and proton proton collision respectively.  $R_{AA}$ quantifies the shift in the spectra from 

Figure \ref{fig:JetEloss} shows the nuclear modification factor with R = 0.4 jets in the ATLAS experiment at 5.02 TeV\cite{Aaboud:2018twu}.  The different colored bands in the figure are broken into centrality
\begin{figure}[h]
